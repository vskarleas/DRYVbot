...
| V6.2.2  | Created a speech node for sending voice commands to the robot over the interface. The speech node was included on the launch logic.launch.py. Also, we updated teh room=command-panel for foxglove in order to do send recording voice requests directly from inside the plugin |
